import type { TranslationKeys } from './lang_es';

export const lang_en: TranslationKeys = {
  systemSettings: {
    sectionTitle: 'System Configuration',
    readOnlyModeLabel: 'Enable Read-Only Mode (Entire System)',
    readOnlyModeDesc: 'When enabled, ALL users will be unable to create episodes or notes when the central system is online',
    newEpisodeButtonLabel: 'Enable "New Episode" button',
    newEpisodeButtonDesc: 'When enabled, the button to create new episodes will be visible to all users',
  },
  readOnlyMode: {
    title: 'Read-Only Mode Active',
    loginAlert: '⚠️ Read-Only Mode Activated\n\nThe system detected a connection to the central server.\n\nDuring this time, you will not be able to:\n• Create new episodes\n• Record clinical notes\n\nYou will only be able to view existing information.',
    episodesBanner: 'The system detected a connection to the central server. During this time, you will only be able to view information. Episode and clinical note creation is disabled.',
    notesBanner: 'The system detected a connection to the central server. You cannot create new clinical notes at this time.',
    buttonTooltip: 'Not available in read-only mode',
    textareaPlaceholder: 'Not available in read-only mode',
  },

  common: {
    loading: 'Loading...',
    error: 'Error',
    success: 'Success',
    cancel: 'Cancel',
    confirm: 'Confirm',
    save: 'Save',
    delete: 'Delete',
    edit: 'Edit',
    search: 'Search',
    close: 'Close',
    yes: 'Yes',
    no: 'No',
  },

  header: {
    logout: 'Logout',
    settings: 'Settings',
    darkMode: 'Dark mode',
    lightMode: 'Light mode',
    language: 'Language',
    sslWarning: 'SSL Warning',
  },

  login: {
    title: 'Login',
    subtitle: 'Enter your credentials to continue',
    username: 'Username',
    password: 'Password',
    usernamePlaceholder: 'Enter your username',
    passwordPlaceholder: 'Enter your password',
    loginButton: 'Login',
    loginError: 'Invalid username or password',
    biometricLogin: 'Sign in with {provider}',
    biometricEnableTitle: 'Enable biometric login',
    biometricEnableMessage: 'Sign in with {provider} next time?',
    biometricEnable: 'Enable',
    biometricNotNow: 'Not now',
    biometricPromptLogin: 'Sign in with biometrics',
    biometricPromptEnable: 'Confirm to enable biometric login',
    biometricLabelFaceId: 'Face ID',
    biometricLabelTouchId: 'Touch ID',
    biometricLabelFingerprint: 'Fingerprint',
    biometricLabelFacial: 'Face recognition',
    biometricLabel: 'Biometrics',
  },

  episodes: {
    title: 'Episodes List',
    newEpisode: 'New Episode',
    viewNote: 'View Note',
    patient: 'Patient',
    mrn: 'MRN',
    run: 'ID',
    episode: 'Episode',
    type: 'Type',
    date: 'Date',
    dateAttention: 'Service Date',
    status: 'Status',
    hospital: 'Hospital',
    location: 'Location',
    professional: 'Professional',
    sync: 'Sync',
    noEpisodes: 'No episodes available',
    noEpisodesInCategory: 'No episodes in this category',
    searchPlaceholder: 'Search by name, RUN, MRN...',
    loadingEpisodes: 'Loading episodes...',
    syncStatus: {
      online: 'Online',
      offline: 'Offline',
      pending: 'Pending sync',
      syncing: 'Syncing...',
      synced: 'Synced',
      pendingCount: 'Pending',
      new: 'New',
      local: 'On device',
    },
  },

  syncPipeline: {
    app: 'App',
    localServer: 'Local',
    centralServer: 'Central',
    linkOk: 'OK',
    lostConnection: 'Lost connection, {ago}',
    pendingInOutbox: '{n} pending',
    pendingEvents: '{n} pending',
    lastSent: 'sent {ago}',
    never: 'never',
    sslWarning: 'SSL Warning',
  },

  clinicalNote: {
    title: 'Clinical Note',
    patientInfo: 'Patient Information',
    backToList: 'Back to list',
    backToEpisodes: 'Back to Episodes',
    saveNote: 'Save Note',
    saving: 'Saving...',
    clinicalRecord: 'Clinical Record',
    patient: 'Patient',
    episodeNumber: 'Episode Number',
    sex: 'Sex',
    unknown: 'Unknown',
    birthDate: 'Date of Birth',
    attentionDate: 'Service Date',
    episodeType: 'Episode Type',
    professional: 'Professional',
    consultReason: 'Reason for Consultation',
    location: 'Location',
    room: 'Room',
    bed: 'Bed',
    previousNotes: 'Previous Clinical Notes',
    newNote: 'New Clinical Note',
    notePlaceholder: 'Write the clinical note here...',
    characters: 'characters',
    sent: 'Sent',
    pending: 'Pending',
    episodeNotFound: 'Episode not found',
    tabs: {
      subjective: 'Subjective',
      objective: 'Objective',
      assessment: 'Assessment',
      plan: 'Plan',
    },
    placeholders: {
      subjective: 'Chief complaint, symptoms reported by the patient...',
      objective: 'Vital signs, physical examination...',
      assessment: 'Diagnostic impression...',
      plan: 'Treatment, indications...',
    },
    saveSuccess: 'Clinical note saved successfully',
    saveError: 'Error saving note',
    loadError: 'Error loading episode data',
    editNote: 'Edit note',
    deleteNote: 'Delete note',
    deleteConfirm: 'Delete this clinical note? This action cannot be undone.',
    editSave: 'Save changes',
    editCancel: 'Cancel',
    cannotEditSynced: 'Cannot edit a note that has already been synced to the central server',
  },

  patientHistory: {
    title: 'Patient History',
    allergies: 'Allergies',
    offlineRecords: 'Offline Records',
    previousEncounters: 'Previous Encounters',
    labResults: 'Lab Results',
    noAllergies: 'No allergies recorded',
    noRecords: 'No offline records',
    noEncounters: 'No previous encounters recorded',
    noResults: 'No exam results',
    allergyTypes: {
      Medicamento: 'Medication',
      Alimento: 'Food',
      Ambiental: 'Environmental',
      Contacto: 'Contact',
    },
    severity: {
      Alta: 'High',
      Moderada: 'Moderate',
      Leve: 'Mild',
      Severa: 'Severe',
      Media: 'Medium',
      Baja: 'Low',
    },
    active: 'Active',
    inactive: 'Inactive',
    viewFullNote: 'Click to view full',
    noteDetails: 'Offline Record Details',
    dateTime: 'Date and Time',
    responsible: 'Responsible',
    note: 'Note',
    fullNote: 'Full Note',
    close: 'Close',
    consultReason: 'Reason for Consultation',
    diagnoses: 'Diagnoses',
    medications: 'Medications',
    indications: 'Indications',
    examType: 'Type',
    exam: 'Exam',
    exams: 'exams',
    value: 'Value',
    referenceRange: 'Reference range',
    doctor: 'Doctor',
    specialty: 'Specialty',
    type: 'Type',
    abnormalFlag: {
      Normal: 'Normal',
      Alto: 'High',
      Bajo: 'Low',
      High: 'High',
      Low: 'Low',
    },
  },

  newEpisode: {
    title: 'New Episode',
    titlePatient: 'New Patient + Episode',
    backToEpisodes: 'Back to Episodes',
    searchPatient: 'Search Patient',
    searchPlaceholder: 'Search by name, ID or MRN',
    selectPatient: 'Select patient to create episode',
    patientInfo: 'Patient Information',
    patientData: 'Patient Data',
    episodeInfo: 'Episode Information',
    episodeData: 'Episode Data',
    createEpisode: 'Create Episode',
    createAndContinue: 'Create and Continue',
    creating: 'Creating...',
    cancel: 'Cancel',
    episodeNumber: 'Episode Number',
    type: 'Type',
    episodeType: 'Episode Type',
    noEpisodeTypes: 'No types available',
    date: 'Date of Service',
    hospital: 'Hospital',
    room: 'Room',
    roomBox: 'Room/Box',
    bed: 'Bed',
    location: 'Location',
    local: 'Local',
    clinicUnit: 'Clinical Unit',
    clinicUnitPlaceholder: 'Select location...',
    clinicUnitRequired: 'A location must be selected',
    clinicUnitNoData: 'No locations available for this episode type',
    clinicUnitNoDataSync: 'No locations available. Sync data from central system.',
    clinicUnitNoResults: 'No locations found',
    status: 'Status',
    professional: 'Professional',
    firstName: 'First Name',
    lastName: 'Last Name',
    rut: 'ID',
    noDocument: 'No Document',
    sex: 'Sex',
    birthDate: 'Date of Birth',
    consultReason: 'Reason for Consultation',
    required: 'This field is required',
    createSuccess: 'Episode created successfully',
    createError: 'Error creating episode',
    sexOptions: {
      M: 'Male',
      F: 'Female',
      O: 'Other',
      U: 'Unknown',
    },
  },

  userSettings: {
    title: 'User Settings',
    filters: 'Filters',
    hospitals: 'Hospitals',
    locations: 'Locations',
    professionals: 'Professionals',
    selectAll: 'Select all',
    deselectAll: 'Deselect all',
    saveSuccess: 'Settings saved successfully',
    saveError: 'Error saving settings',
    tabSettings: 'Settings',
    tabUsers: 'User Management',
    usernameLabel: 'Username',
    fullNameLabel: 'Full Name',
    fullNamePlaceholder: 'Your full name',
    fullNameHint: 'This name will be recorded when you create clinical notes',
    newPasswordLabel: 'New Password',
    newPasswordPlaceholder: 'Leave blank to keep unchanged',
    confirmPasswordLabel: 'Confirm Password',
    confirmPasswordPlaceholder: 'Repeat new password',
    apiFiltersLabel: 'API Filters',
    apiFiltersHint: 'Query parameters for the obtenerDatos endpoint',
    apiFiltersPlaceholder: 'e.g.: filter1=value1&filter2=value2',
    systemUsersTitle: 'System Users',
    userSearchPlaceholder: 'Search by username or full name...',
    noUsersFound: 'No users found',
    adminBadge: 'Admin',
    statusLabel: 'Status',
    statusActive: 'Active',
    statusInactive: 'Inactive',
    createUserButton: 'Create New User',
    newUserTitle: 'New User',
    newUsernamePlaceholder: 'Username',
    newFullNamePlaceholder: 'Full name (optional)',
    isAdminLabel: 'Is administrator',
    isAdminHint: 'Administrators can manage users and system settings',
    createUserSubmitButton: 'Create User',
    createUserSuccess: 'User created successfully',
    createUserError: 'Error creating user',
    passwordsNoMatch: 'Passwords do not match',
    noChanges: 'No changes to save',
    credentialsRequired: 'Username and password are required',
    filterOruUserLabel: 'ORU User',
    filterOruUserPlaceholder: 'e.g.: dr.garcia',
    filterOruUserHint: 'Included in the OBR.24 field when sending HL7 ORU messages',
    filterEpisodeSection: 'Episode Filters',
    filterTipo: 'Type',
    filterTipoPlaceholder: 'e.g.: Ambulatorio',
    filterHospital: 'Hospital',
    filterHospitalPlaceholder: 'e.g.: Hospital Base',
    filterLocal: 'Local / Unit',
    filterLocalPlaceholder: 'e.g.: Unidad Emergencia Adultos',
    filterProfesional: 'Professional',
    filterProfesionalPlaceholder: 'e.g.: Dr. Juan Pérez',
    loadMoreUsers: 'Load more users',
    tabFilters: 'Filters',
    tabTexts: 'Predefined Texts',
    tabSistema: 'System',
  },

  episodeTypes: {
    'Urgencia': 'Emergency',
    'Ambulatorio': 'Outpatient',
    'Hospitalizado': 'Inpatient',
    'Pabellón': 'Operating Room',
    'Emergencia': 'Emergency',
  },

  episodeStatus: {
    'Activo': 'Active',
    'En Atención': 'In Care',
    'Finalizado': 'Completed',
    'Pendiente': 'Pending',
  },

  offline: {
    queuedNote: 'Note queued — will be sent on reconnect',
  },

  serverDiscovery: {
    title: 'Connect to Server',
    subtitle: 'Enter the TrakCare server address',
    currentServer: 'Current server:',
    tryingDefault: 'Connecting to',
    serverAddress: 'Server Address',
    manualPlaceholder: 'fcoffline.local:8000',
    manualHint: 'Enter IP or hostname. Port 8000 and http:// protocol are added automatically if not specified.',
    testAndConnect: 'Test & Connect',
    connectionFailed: 'Could not connect to the server. Check the address and make sure the server is running.',
    changeServer: 'Change Server',
    continueOffline: 'Continue without connection (offline mode)',
    resumeLastConnection: 'Resume last connection (offline mode)',
  },

  speech: {
    start: 'Dictate',
    stop: 'Stop dictation',
    listening: 'Listening...',
    permissionDenied: 'Microphone permission denied',
    unsupported: 'Dictation not available on this device or browser',
    errorGeneric: 'Speech recognition error',
    command: {
      start: 'Command dictation',
      stop: 'Stop command dictation',
      hint: 'Say the field name followed by the value. E.g. "first name John last name Smith sex male type emergency".',
      activeField: 'Active field',
      noMatch: 'No field recognized',
      next: ['next', 'next field'],
      fields: {
        firstName: ['first name', 'name', 'given name'],
        lastName: ['last name', 'surname', 'family name'],
        rut: ['rut', 'run', 'document', 'id', 'identification'],
        noDocument: ['no id', 'no document', 'without id', 'no rut'],
        birthDate: ['birth date', 'date of birth', 'birthday', 'born'],
        sex: ['sex', 'gender'],
        episodeType: ['type', 'episode type', 'episode'],
        roomBox: ['room', 'box', 'bed'],
        clinicUnit: ['unit', 'location', 'clinical unit', 'ward'],
        motivoConsulta: ['reason', 'consult reason', 'chief complaint', 'complaint'],
      },
      sexValues: {
        M: ['male', 'man', 'boy'],
        F: ['female', 'woman', 'girl'],
        O: ['other', 'non binary', 'nonbinary'],
        U: ['unknown', 'unspecified', 'not specified'],
      },
    },
  },

  predefinedTexts: {
    tab: 'Texts',
    desc: 'Predefined texts to insert into clinical notes',
    addText: 'Add text',
    noTexts: 'No predefined texts',
    titlePlaceholder: 'Text title',
    contentPlaceholder: 'Text content...',
    deleteConfirm: 'Delete this predefined text?',
    insertText: 'Insert',
    active: 'Active',
    inactive: 'Inactive',
    save: 'Save',
    cancel: 'Cancel',
    edit: 'Edit',
    delete: 'Delete',
  },

  adminConfig: {
    sectionTitle: 'Advanced Configuration',
    groupConnection: 'Central Server',
    groupCredentials: 'Credentials',
    groupIntervals: 'Intervals',
    centralUrl: 'Central Server URL',
    centralApiEndpoint: 'Central API Endpoint',
    centralHl7Endpoint: 'Central HL7 Endpoint',
    centralUsersEndpoint: 'Central Users Endpoint',
    centralApiUsername: 'Central API Username',
    centralApiPassword: 'Central API Password',
    healthCheckInterval: 'Health check interval (s)',
    downstreamSyncInterval: 'Downstream sync interval (s)',
    upstreamSyncInterval: 'Upstream sync interval (s)',
    maxRetries: 'Max retries',
    saveSuccess: 'Configuration saved successfully',
    saveError: 'Error saving configuration',
    effectNote: 'Changes take effect on the next synchronization cycle',
  },
};
