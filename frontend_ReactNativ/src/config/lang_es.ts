export const lang_es = {
  systemSettings: {
    sectionTitle: 'Configuración del Sistema',
    readOnlyModeLabel: 'Habilitar Modo Solo Lectura (Sistema Completo)',
    readOnlyModeDesc: 'Cuando está habilitado, NINGÚN usuario podrá crear episodios ni notas cuando el sistema central esté en línea',
    newEpisodeButtonLabel: 'Habilitar botón "Nuevo Episodio"',
    newEpisodeButtonDesc: 'Cuando está habilitado, el botón para crear nuevos episodios será visible para todos los usuarios',
  },
  readOnlyMode: {
    title: 'Modo Solo Lectura Activo',
    loginAlert: '⚠️ Modo Solo Lectura Activado\n\nEl sistema detectó conexión con el servidor central.\n\nDurante este tiempo, no podrá:\n• Crear nuevos episodios\n• Registrar notas clínicas\n\nSolo podrá consultar información existente.',
    episodesBanner: 'El sistema detectó conexión con el servidor central. Durante este tiempo, solo podrá consultar información. La creación de episodios y notas clínicas está deshabilitada.',
    notesBanner: 'El sistema detectó conexión con el servidor central. No podrá crear nuevas notas clínicas en este momento.',
    buttonTooltip: 'No disponible en modo solo lectura',
    textareaPlaceholder: 'No disponible en modo solo lectura',
  },
  common: {
    loading: 'Cargando...',
    error: 'Error',
    success: 'Éxito',
    cancel: 'Cancelar',
    confirm: 'Confirmar',
    save: 'Guardar',
    delete: 'Eliminar',
    edit: 'Editar',
    search: 'Buscar',
    close: 'Cerrar',
    yes: 'Sí',
    no: 'No',
  },

  header: {
    logout: 'Cerrar sesión',
    settings: 'Configuración',
    darkMode: 'Modo oscuro',
    lightMode: 'Modo claro',
    language: 'Idioma',
    sslWarning: 'Advertencia SSL',
  },

  login: {
    title: 'Iniciar sesión',
    subtitle: 'Ingrese sus credenciales para continuar',
    username: 'Usuario',
    password: 'Contraseña',
    usernamePlaceholder: 'Ingrese su usuario',
    passwordPlaceholder: 'Ingrese su contraseña',
    loginButton: 'Ingresar',
    loginError: 'Usuario o contraseña incorrectos',
    biometricLogin: 'Iniciar con {provider}',
    biometricEnableTitle: 'Activar inicio biométrico',
    biometricEnableMessage: '¿Desea iniciar sesión con {provider} la próxima vez?',
    biometricEnable: 'Activar',
    biometricNotNow: 'Ahora no',
    biometricPromptLogin: 'Iniciar sesión con biometría',
    biometricPromptEnable: 'Confirmar para activar inicio biométrico',
    biometricLabelFaceId: 'Face ID',
    biometricLabelTouchId: 'Touch ID',
    biometricLabelFingerprint: 'Huella digital',
    biometricLabelFacial: 'Reconocimiento facial',
    biometricLabel: 'Biometría',
  },

  episodes: {
    title: 'Lista de Episodios',
    newEpisode: 'Nuevo Episodio',
    viewNote: 'Ver Nota',
    patient: 'Paciente',
    mrn: 'MRN',
    run: 'RUN',
    episode: 'Episodio',
    type: 'Tipo',
    date: 'Fecha',
    dateAttention: 'Fecha Atención',
    status: 'Estado',
    hospital: 'Hospital',
    location: 'Ubicación',
    professional: 'Profesional',
    sync: 'Sincronización',
    noEpisodes: 'No hay episodios disponibles',
    noEpisodesInCategory: 'No hay episodios en esta categoría',
    searchPlaceholder: 'Buscar por nombre, RUN, MRN...',
    loadingEpisodes: 'Cargando episodios...',
    syncStatus: {
      online: 'Conectado',
      offline: 'Sin conexión',
      pending: 'Pendiente de sincronización',
      syncing: 'Sincronizando...',
      synced: 'Sincronizado',
      pendingCount: 'Pendiente',
      new: 'Nuevo',
      local: 'En dispositivo',
    },
  },

  syncPipeline: {
    app: 'App',
    localServer: 'Local',
    centralServer: 'Central',
    linkOk: 'OK',
    lostConnection: 'Sin conexión, {ago}',
    pendingInOutbox: '{n} pendientes',
    pendingEvents: '{n} pendientes',
    lastSent: 'enviado {ago}',
    never: 'nunca',
    sslWarning: 'Advertencia SSL',
  },

  clinicalNote: {
    title: 'Nota Clínica',
    patientInfo: 'Información del Paciente',
    backToList: 'Volver a lista',
    backToEpisodes: 'Volver a Episodios',
    saveNote: 'Guardar Nota',
    saving: 'Guardando...',
    clinicalRecord: 'Registro Clínico',
    patient: 'Paciente',
    episodeNumber: 'Número de Episodio',
    sex: 'Sexo',
    unknown: 'Desconocido',
    birthDate: 'Fecha de Nacimiento',
    attentionDate: 'Fecha de Atención',
    episodeType: 'Tipo de Episodio',
    professional: 'Profesional',
    consultReason: 'Motivo de Consulta',
    location: 'Ubicación',
    room: 'Habitación',
    bed: 'Cama',
    previousNotes: 'Notas Clínicas Anteriores',
    newNote: 'Nueva Nota Clínica',
    notePlaceholder: 'Escriba aquí la nota clínica...',
    characters: 'caracteres',
    sent: 'Enviado',
    pending: 'Pendiente',
    episodeNotFound: 'Episodio no encontrado',
    tabs: {
      subjective: 'Subjetivo',
      objective: 'Objetivo',
      assessment: 'Evaluación',
      plan: 'Plan',
    },
    placeholders: {
      subjective: 'Motivo de consulta, síntomas referidos por el paciente...',
      objective: 'Signos vitales, examen físico...',
      assessment: 'Impresión diagnóstica...',
      plan: 'Tratamiento, indicaciones...',
    },
    saveSuccess: 'Nota clínica guardada exitosamente',
    saveError: 'Error al guardar la nota',
    loadError: 'Error al cargar los datos del episodio',
    editNote: 'Editar nota',
    deleteNote: 'Eliminar nota',
    deleteConfirm: '¿Eliminar esta nota clínica? Esta acción no se puede deshacer.',
    editSave: 'Guardar cambios',
    editCancel: 'Cancelar',
    cannotEditSynced: 'No se puede editar una nota ya sincronizada con el servidor central',
  },

  patientHistory: {
    title: 'Antecedentes del Paciente',
    allergies: 'Alergias',
    offlineRecords: 'Registros Offline',
    previousEncounters: 'Encuentros Previos',
    labResults: 'Resultados de Exámenes',
    noAllergies: 'No hay alergias registradas',
    noRecords: 'No hay registros offline',
    noEncounters: 'No hay encuentros previos registrados',
    noResults: 'No hay resultados de exámenes',
    allergyTypes: {
      Medicamento: 'Medicamento',
      Alimento: 'Alimento',
      Ambiental: 'Ambiental',
      Contacto: 'Contacto',
    },
    severity: {
      Alta: 'Alta',
      Moderada: 'Moderada',
      Leve: 'Leve',
      Severa: 'Severa',
      Media: 'Media',
      Baja: 'Baja',
    },
    active: 'Activa',
    inactive: 'Inactiva',
    viewFullNote: 'Click para ver completo',
    noteDetails: 'Detalle de Registro Offline',
    dateTime: 'Fecha y Hora',
    responsible: 'Responsable',
    note: 'Nota',
    fullNote: 'Nota Completa',
    close: 'Cerrar',
    consultReason: 'Motivo de Consulta',
    diagnoses: 'Diagnósticos',
    medications: 'Medicamentos',
    indications: 'Indicaciones',
    examType: 'Tipo',
    exam: 'Examen',
    exams: 'exámenes',
    value: 'Valor',
    referenceRange: 'Rango de referencia',
    doctor: 'Médico',
    specialty: 'Especialidad',
    type: 'Tipo',
    abnormalFlag: {
      Normal: 'Normal',
      Alto: 'Alto',
      Bajo: 'Bajo',
      High: 'Alto',
      Low: 'Bajo',
    },
  },

  newEpisode: {
    title: 'Nuevo Episodio',
    titlePatient: 'Nuevo Paciente + Episodio',
    backToEpisodes: 'Volver a Episodios',
    searchPatient: 'Buscar Paciente',
    searchPlaceholder: 'Buscar por nombre, RUN o MRN',
    selectPatient: 'Seleccionar paciente para crear episodio',
    patientInfo: 'Información del Paciente',
    patientData: 'Datos del Paciente',
    episodeInfo: 'Información del Episodio',
    episodeData: 'Datos del Episodio',
    createEpisode: 'Crear Episodio',
    createAndContinue: 'Crear y Continuar',
    creating: 'Creando...',
    cancel: 'Cancelar',
    episodeNumber: 'Número de Episodio',
    type: 'Tipo',
    episodeType: 'Tipo de Episodio',
    noEpisodeTypes: 'No hay tipos disponibles',
    date: 'Fecha de Atención',
    hospital: 'Hospital',
    room: 'Habitación',
    roomBox: 'Habitación/Box',
    bed: 'Cama',
    location: 'Ubicación',
    local: 'Local',
    clinicUnit: 'Unidad Clínica',
    clinicUnitPlaceholder: 'Seleccionar ubicación...',
    clinicUnitRequired: 'Debe seleccionar una ubicación',
    clinicUnitNoData: 'No hay ubicaciones disponibles para este tipo de episodio',
    clinicUnitNoDataSync: 'No hay ubicaciones disponibles. Sincronice datos del sistema central.',
    clinicUnitNoResults: 'No se encontraron ubicaciones',
    status: 'Estado',
    professional: 'Profesional',
    firstName: 'Nombre',
    lastName: 'Apellidos',
    rut: 'RUT',
    noDocument: 'Sin Documento',
    sex: 'Sexo',
    birthDate: 'Fecha de Nacimiento',
    birthDateRequired: 'La fecha de nacimiento es requerida',
    required: 'Este campo es requerido',
    createSuccess: 'Episodio creado exitosamente',
    createError: 'Error al crear el episodio',
    sexOptions: {
      M: 'Masculino',
      F: 'Femenino',
      O: 'Otro',
      U: 'Desconocido',
    },
  },

  userSettings: {
    title: 'Configuración de Usuario',
    filters: 'Filtros',
    hospitals: 'Hospitales',
    locations: 'Ubicaciones',
    professionals: 'Profesionales',
    selectAll: 'Seleccionar todos',
    deselectAll: 'Deseleccionar todos',
    saveSuccess: 'Configuración guardada exitosamente',
    saveError: 'Error al guardar la configuración',
    tabSettings: 'Configuración',
    tabUsers: 'Gestión de Usuarios',
    usernameLabel: 'Usuario',
    fullNameLabel: 'Nombre Completo',
    fullNamePlaceholder: 'Tu nombre completo',
    fullNameHint: 'Este nombre se registrará cuando crees notas clínicas',
    newPasswordLabel: 'Nueva Contraseña',
    newPasswordPlaceholder: 'Dejar en blanco para no cambiar',
    confirmPasswordLabel: 'Confirmar Contraseña',
    confirmPasswordPlaceholder: 'Repetir nueva contraseña',
    apiFiltersLabel: 'Filtros API',
    apiFiltersHint: 'Parámetros de consulta para el endpoint obtenerDatos',
    apiFiltersPlaceholder: 'ej: filtro1=valor1&filtro2=valor2',
    systemUsersTitle: 'Usuarios del Sistema',
    userSearchPlaceholder: 'Buscar por usuario o nombre completo...',
    noUsersFound: 'No se encontraron usuarios',
    adminBadge: 'Administrador',
    statusLabel: 'Estado',
    statusActive: 'Activo',
    statusInactive: 'Inactivo',
    createUserButton: 'Crear Nuevo Usuario',
    newUserTitle: 'Nuevo Usuario',
    newUsernamePlaceholder: 'Nombre de usuario',
    newFullNamePlaceholder: 'Nombre completo (opcional)',
    isAdminLabel: 'Es administrador',
    isAdminHint: 'Los administradores pueden gestionar usuarios y configuración del sistema',
    createUserSubmitButton: 'Crear Usuario',
    createUserSuccess: 'Usuario creado correctamente',
    createUserError: 'Error al crear usuario',
    passwordsNoMatch: 'Las contraseñas no coinciden',
    noChanges: 'No hay cambios para guardar',
    credentialsRequired: 'Usuario y contraseña son requeridos',
    filterOruUserLabel: 'Usuario ORU',
    filterOruUserPlaceholder: 'ej: dr.garcia',
    filterOruUserHint: 'Se incluye en el campo OBR.24 al enviar mensajes HL7 ORU',
    filterEpisodeSection: 'Filtros de Episodios',
    filterTipo: 'Tipo',
    filterTipoPlaceholder: 'ej: Ambulatorio',
    filterHospital: 'Hospital',
    filterHospitalPlaceholder: 'ej: Hospital Base',
    filterLocal: 'Local / Unidad',
    filterLocalPlaceholder: 'ej: Unidad Emergencia Adultos',
    filterProfesional: 'Profesional',
    filterProfesionalPlaceholder: 'ej: Dr. Juan Pérez',
    loadMoreUsers: 'Ver más usuarios',
    tabFilters: 'Filtros',
    tabTexts: 'Textos Predefinidos',
    tabSistema: 'Sistema',
  },

  episodeTypes: {
    'Urgencia': 'Urgencia',
    'Ambulatorio': 'Ambulatorio',
    'Hospitalizado': 'Hospitalizado',
    'Pabellón': 'Pabellón',
    'Emergencia': 'Emergencia',
  },

  episodeStatus: {
    'Activo': 'Activo',
    'En Atención': 'En Atención',
    'Finalizado': 'Finalizado',
    'Pendiente': 'Pendiente',
  },

  offline: {
    queuedNote: 'Nota en cola — se enviará al reconectar',
  },

  serverDiscovery: {
    title: 'Conectar al Servidor',
    subtitle: 'Ingrese la dirección del servidor TrakCare',
    currentServer: 'Servidor actual:',
    tryingDefault: 'Conectando a',
    serverAddress: 'Dirección del Servidor',
    manualPlaceholder: 'fcoffline.local:8000',
    manualHint: 'Ingrese IP o hostname. El puerto 8000 y protocolo http:// se agregan automáticamente si no se especifican.',
    testAndConnect: 'Probar y Conectar',
    connectionFailed: 'No se pudo conectar al servidor. Verifique la dirección y que el servidor esté corriendo.',
    changeServer: 'Cambiar Servidor',
    continueOffline: 'Continuar sin conexión (modo offline)',
    resumeLastConnection: 'Retomar última conexión (modo offline)',
  },

  speech: {
    start: 'Dictar',
    stop: 'Detener dictado',
    listening: 'Escuchando...',
    permissionDenied: 'Permiso de micrófono denegado',
    unsupported: 'Dictado no disponible en este dispositivo o navegador',
    errorGeneric: 'Error en el reconocimiento de voz',
    command: {
      start: 'Dictado por comandos',
      stop: 'Detener dictado por comandos',
      hint: 'Diga el nombre del campo seguido del valor. Ej: "nombre Juan apellidos Pérez sexo masculino tipo urgencia".',
      activeField: 'Campo activo',
      noMatch: 'No se reconoció ningún campo',
      // Meta keywords: switch to the next field in declared order.
      next: ['siguiente', 'siguiente campo', 'próximo', 'próximo campo'],
      // Field aliases: spoken keywords that switch the active field.
      // The first alias is also used as the display name.
      fields: {
        firstName: ['nombre', 'nombres', 'primer nombre'],
        lastName: ['apellido', 'apellidos', 'segundo apellido'],
        rut: ['rut', 'run', 'documento', 'identificación', 'cédula'],
        noDocument: ['sin documento', 'sin documentos', 'sin rut', 'no id', 'sin id', 'no documento'],
        birthDate: ['fecha de nacimiento', 'nacimiento', 'fecha nacimiento', 'cumpleaños'],
        sex: ['sexo', 'género'],
        episodeType: ['tipo de episodio', 'tipo', 'episodio'],
        roomBox: ['habitación', 'box', 'sala', 'cama'],
        clinicUnit: ['unidad clínica', 'ubicación', 'unidad', 'servicio'],
        motivoConsulta: ['motivo', 'motivo de consulta', 'motivo de la consulta', 'razón'],
      },
      // Value aliases for fixed-option pickers.
      sexValues: {
        M: ['masculino', 'hombre', 'varón', 'macho'],
        F: ['femenino', 'mujer', 'hembra'],
        O: ['otro', 'otra', 'no binario'],
        U: ['desconocido', 'no especificado', 'sin especificar'],
      },
    },
  },

  predefinedTexts: {
    tab: 'Textos',
    desc: 'Textos predefinidos para insertar en notas clínicas',
    addText: 'Agregar texto',
    noTexts: 'No hay textos predefinidos',
    titlePlaceholder: 'Título del texto',
    contentPlaceholder: 'Contenido del texto...',
    deleteConfirm: '¿Eliminar este texto predefinido?',
    insertText: 'Insertar',
    active: 'Activo',
    inactive: 'Inactivo',
    save: 'Guardar',
    cancel: 'Cancelar',
    edit: 'Editar',
    delete: 'Eliminar',
  },

  adminConfig: {
    sectionTitle: 'Configuración avanzada',
    groupConnection: 'Servidor Central',
    groupCredentials: 'Credenciales',
    groupIntervals: 'Intervalos',
    centralUrl: 'URL del Servidor Central',
    centralApiEndpoint: 'Endpoint API Central',
    centralHl7Endpoint: 'Endpoint HL7 Central',
    centralUsersEndpoint: 'Endpoint Usuarios Central',
    centralApiUsername: 'Usuario API Central',
    centralApiPassword: 'Contraseña API Central',
    healthCheckInterval: 'Intervalo de verificación (s)',
    downstreamSyncInterval: 'Intervalo sync descendente (s)',
    upstreamSyncInterval: 'Intervalo sync ascendente (s)',
    maxRetries: 'Reintentos máximos',
    saveSuccess: 'Configuración guardada exitosamente',
    saveError: 'Error al guardar la configuración',
    effectNote: 'Los cambios toman efecto en el siguiente ciclo de sincronización',
  },
};

export type TranslationKeys = typeof lang_es;
